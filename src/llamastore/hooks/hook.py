...
class CustomHook:
    def before_request(self, request: Request):
        print(
            f"Before request on URL {request.url} with method {request.method.upper()}"
        )

    def after_response(self, request: Request, response: Response):
        print(
            f"After response on URL {request.url} with method {request.method.upper()}, returning status {response.status}"
        )
